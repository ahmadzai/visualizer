...
use AppBundle\Entity\Covid19Cases;
...
class CasesController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $totalCases = $em->getRepository('AppBundle:Covid19Cases')->totalCases();
        $casesByRegion = $em->getRepository('AppBundle:Covid19Cases')->casesByRegion();
        $covid19Cases = $em->getRepository('AppBundle:Covid19Cases')->findAll();

        $lastUpdated = $em->getRepository("AppBundle:Covid19Cases")->getLastUpdated();
     ...
     * @Security("has_role('ROLE_COVID19_EDITOR')")
     */
    public function newAction(Request $request)
    {
        $covid19Case = new Covid19Cases();
        $form = $this->createForm('AppBundle\Form\Covid19CasesType', $covid19Case);
    ...
    public function showAction(Covid19Cases $covid19Case)
    {
        ...
        $em = $this->getDoctrine()->getManager();
        $totalCases = $em->getRepository('AppBundle:Covid19Cases')->totalCases($province);
    ...
    {
        $deleteForm = $this->createDeleteForm($covid19Case);
        $editForm = $this->createForm('AppBundle\Form\Covid19CasesType', $covid19Case);
    ...
    public function getCasesForMap(Request $request)
    {
        ...
        $em = $this->getDoctrine()->getManager();
        $cases = $em->getRepository('AppBundle:Covid19Cases')->selectForMap($province);
        //dump($cases); die;
        return new JsonResponse(['map_data' => json_encode($cases)]);
    }

    /**
     * @Route("/load/geojson", name="load_geo_json", options={"expose"=true})
     */
    public function loadMapShapeFile() {
        $mapData = Maps::loadGeoJson($this->getParameter('kernel.root_dir'), "province");
