 ...
 */
class Covid19CasesRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function totalCases(Province $province = null) {
        ...
        $query = $this ->getEntityManager()
            ...
            SUM(total.noOfRecoveredCases) as totalRecovered, SUM(total.noOfDeaths) as totalDeath FROM AppBundle:Covid19Cases total 
             '.$condition);
        if($province)
    ...
    public function casesByRegion() {
        return $this->getEntityManager()
            ...
            SUM(total.noOfRecoveredCases) as totalRecovered, SUM(total.noOfDeaths) as totalDeath
                     FROM AppBundle:Covid19Cases total JOIN total.province p GROUP BY p.provinceRegion
    ...
    public function selectForMap(int $province = null) {
        ...
        $query = $this->getEntityManager()
            ...
            total.noOfRecoveredCases as Recovered, total.noOfDeaths as Death
                     FROM AppBundle:Covid19Cases total JOIN total.province p ".$condition);
        if($province)
            $query->setParameter('prov', $province);
